  ...
  return {
    ...
    applyFilters(topics, config, readTopicIds) {
      ...
      if (settings.categoryFilter === 'bookmarks') {
        const bookmarkedIds = bookmarksManager.getBookmarkedIds();
        res = res.filter(t => bookmarkedIds.has(t.id));
      }
    ...
    showContextMenu(e, topicId) {
      ...
      const url = `https://linux.do/t/${topicId}`;
      const isBookmarked = bookmarksManager.isBookmarked(topicId);
      const bookmarkText = isBookmarked ? '取消收藏' : '收藏此主题';
      ...
      menu.innerHTML = `
        ...
        <div class="menu-item" onclick="window.__toggleBookmark(${topicId})">${bookmarkText}</div>
      ...
      menu.style.top = `${Math.min(e.pageY, window.innerHeight - 140)}px`;
    },
